...
Alternatively, we can solve for the "end point" of the uniform distribution i.e. $u$ such that $F_{x}^{-1}(u)\in[0,B]$. Given the cdf of exponential distribution, $F_{x}(u)=1-e^{-yu}$, the inverse cdf is $F_{x}^{-1}(u)=-\frac{1}{y}log(1-u)$. Letting $B=-\frac{1}{y}log(1-u)$ and solving for $u$ gives $u=1-e^{-yB}=F_{x}(B)$. Therefore, we can draw uniform random numbers from the interval $[0,u]$ where $u$ is the value of the CDF of an exponential distribution with rate $\lambda$ evaluated at point $B$.  We then plug those uniform random draws back into the inverse CDF of the exponential distribution with rate $\lambda$ to sample from the conditional distribution.
...
sample_conditional_distribution <- function(lambda, B = 5){
  ...
  # Equivalently, we can do the inverse tranform with [0,1] uniform by adjusting the conditional density
